function Z=runmed(Y, WinLen, EndPts)
%
% USE: [Z]=runmed(Y, WinLen, EndPt)
% 
% Calculate running median. NaNs are ignored. Requires R2015a or 
% newer, or the function nanmean() for older Matlab releases.
%
% Inputs
%    Y: input vector (1D) 
%    WinLen: window length for running median (must be odd integer)
...
%For Matlab 2015a and newer use regular median() function. This is almost an 
...
if ~verLessThan('matlab', '8.5')
    ...
    for i=1+W:N-W
        Z(i)=median(Y(i-W:i+W), 'omitnan');	% Compute median in window
    end
    %shrink window at end-points if requested
    if strcmpi(EndPts, 'ShrinkWin')
        for i=1:W
            Z(i)=median(Y(1:2*i-1), 'omitnan'); %shrink left endpoint
        end
        for i=N-W+1:N
            Z(i)=median(Y(2*i-N:N), 'omitnan'); %shrink right window
        end
    end
...
%For older versions of Matlab use nanmean() instead, but this is slow
    ...
    %loop through each point (but not the endpoints)
    for i=1+W:N-W
        Z(i)=nanmean(Y(i-W:i+W));	% Compute median in window
    end
    %shrink window at end-points if requested
    if strcmpi(EndPts, 'ShrinkWin')
        for i=1:W
